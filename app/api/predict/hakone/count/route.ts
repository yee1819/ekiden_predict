...
export async function GET(req: Request) {
  try {
    ...
    if (schoolId || schoolName) {
      ...
      const count = await prisma.ekiden_Team_Predict.count({ where: { Ekiden_no_teamId: team.id, Ekiden_thId: ekidenThId } })
      return NextResponse.json({ count })
    }

    const count = await prisma.ekiden_Team_Predict.count({ where: { Ekiden_thId: ekidenThId } })
    return NextResponse.json({ count })
  } catch {
