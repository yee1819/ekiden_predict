...
export default async function Page({ params, searchParams }: { params: Promise<{ th: string }>, searchParams: Promise<{ sort?: string, interval?: string, limit?: string }> }) {
    ...
    const key = cacheKey("ekiden-izumo-rank", { th: thNum })
    ...
    try {
        ...
        return <RankView title={`出雲驿传 第${cached.thEntry.ekiden_th}届（${cached.thEntry.year}）区间排行榜`} teamViews={cached.teamViews} orders={cached.orders} intervalMeta={cached.intervalMeta} initialSort={sortKey as any} initialInterval={intervalKey} initialLimit={limitKey} />
    } catch { }
    ...
    const thEntry = await prisma.ekiden_th.findFirst({
        where: {
            ekiden_th: thNum,
            OR: [
                { ekiden: { is: { name: { contains: "出雲" } } } },
                { ekiden: { is: { name: { contains: "出云" } } } },
                { ekiden: { is: { name: { contains: "Izumo" } } } },
                { ekiden: { is: { name: { contains: "IZuMo" } } } },
            ],
        },
        include: { ekiden: true },
    ...
    if (!thEntry) {
        return <div style={{ padding: 16 }}>未找到该届出雲驿传</div>
    }
    ...
    return <RankView title={`出雲驿传 第${thEntry.ekiden_th}届（${thEntry.year}）区间排行榜`} teamViews={teamViews} orders={orders} intervalMeta={intervalMeta} initialSort={sortKey as any} initialInterval={intervalKey} initialLimit={limitKey} />
}
