...
function SchoolZenNiHon({ id, year }: { id?: number | string, year?: number | string }) {
    ...
    useEffect(() => {
        ; (async () => {
            ...
            const zen = ekidens.find((e: any) => e.name === "全日本")
            if (!zen) return
            const eds = await fetchPublicOrApi<any[]>("public-editions", Number(zen.id), `/api/editions?ekidenId=${zen.id}`)
            ...
            const ivs = await fetchPublicOrApi<any[]>("public-intervals", Number(zen.id), `/api/intervals?ekidenId=${zen.id}`)
    ...
    return (
        <div className="rounded-xl border bg-white p-4 shadow-sm">
            <div className="flex items-baseline justify-between">
                <div className="text-lg font-bold">{teamView.schoolName}（全日本 第{edition.ekiden_th}届 / {edition.year}）</div>
